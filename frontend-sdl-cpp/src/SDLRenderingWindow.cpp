...
void SDLRenderingWindow::Fullscreen()
{
    ...
    SDL_GetWindowPosition(_renderingWindow, &_lastWindowLeft, &_lastWindowTop);
    _lastWindowBordered = !(SDL_GetWindowFlags(_renderingWindow) & SDL_WINDOW_BORDERLESS);
    _lastWindowMaximized = !!(SDL_GetWindowFlags(_renderingWindow) & SDL_WINDOW_MAXIMIZED);
    ...
    {
        // Borderless-window fullscreen approach.
        // We intentionally do NOT use SDL_WINDOW_FULLSCREEN_DESKTOP because it triggers
        // a DWM compositor mode switch on Windows that causes performance degradation.
        // We also avoid exactly matching the display bounds, as Windows treats
        // borderless windows at exact display size as "fullscreen" and changes the
        // presentation path — which can hurt performance on multi-monitor setups
        // and Intel GPUs. Making the window 1px shorter avoids this.
        int displayIndex = SDL_GetWindowDisplayIndex(_renderingWindow);
        if (displayIndex < 0) displayIndex = 0;

        SDL_Rect displayBounds;
        SDL_GetDisplayBounds(displayIndex, &displayBounds);

        SDL_SetWindowFullscreen(_renderingWindow, 0); // clear any SDL fullscreen flag
        SDL_RestoreWindow(_renderingWindow);
        SDL_SetWindowBordered(_renderingWindow, SDL_FALSE);
        SDL_SetWindowPosition(_renderingWindow, displayBounds.x, displayBounds.y);
        // Use display height - 1 to prevent Windows from engaging its
        // fullscreen optimization path, which can cause stuttering
        SDL_SetWindowSize(_renderingWindow, displayBounds.w, displayBounds.h - 1);
        SDL_RaiseWindow(_renderingWindow);

        if (_logger.debug())
        {
            int dw{0}, dh{0};
            SDL_GL_GetDrawableSize(_renderingWindow, &dw, &dh);
            poco_debug_f4(_logger, "Borderless fullscreen on display %?d: window %dx%d, drawable %dx%d",
                          displayIndex, displayBounds.w, displayBounds.h, dw);
        }
    }

    // Reapply VSync — mode transitions can reset the swap interval
    UpdateSwapInterval();
...
void SDLRenderingWindow::Windowed()
{
    ...
    SDL_SetWindowBordered(_renderingWindow, _lastWindowBordered ? SDL_TRUE :
        (_config->getBool("borderless", false) ? SDL_FALSE : SDL_TRUE));

    if (_lastWindowMaximized)
    {
        SDL_MaximizeWindow(_renderingWindow);
    }
    else if (_lastWindowWidth > 0 && _lastWindowHeight > 0)
    {
        SDL_SetWindowSize(_renderingWindow, _lastWindowWidth, _lastWindowHeight);
        if (_lastWindowLeft != 0 || _lastWindowTop != 0)
        {
            SDL_SetWindowPosition(_renderingWindow, _lastWindowLeft, _lastWindowTop);
        }
    }
