...
bool FileChooser::PopulateFileList()
{
    ...
    for (const auto& file : _currentFileList)
    {
        ...
        // Append unique index to avoid ImGui ID conflicts for same-named items
        std::string selectableId = filename + "###fileitem" + std::to_string(index);

        if (ImGui::Selectable(selectableId.c_str(), isSelected, ImGuiSelectableFlags_AllowDoubleClick))
        {
