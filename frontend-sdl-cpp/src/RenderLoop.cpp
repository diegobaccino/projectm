...
void RenderLoop::LoadPersistedLogoConfig()
{
    try
    {
        std::string logoPath = _userConfig->getString("logo.path", "");
        if (logoPath.empty())
        {
            return;
        }

        if (!projectm_logo_overlay_load_image(_projectMHandle, logoPath.c_str()))
        {
            poco_warning_f1(_logger, "Failed to load persisted logo: %s", logoPath);
            return;
        }

        poco_information_f1(_logger, "Loaded persisted logo: %s", logoPath);

        bool enabled = _userConfig->getBool("logo.enabled", true);
        projectm_logo_overlay_set_enabled(_projectMHandle, enabled);

        float opacity = static_cast<float>(_userConfig->getDouble("logo.opacity", 0.8));
        projectm_logo_overlay_set_opacity(_projectMHandle, opacity);

        float scale = static_cast<float>(_userConfig->getDouble("logo.scale", 0.15));
        projectm_logo_overlay_set_size(_projectMHandle, scale);
        ...
        projectm_logo_overlay_set_reactive_effect(_projectMHandle, static_cast<projectm_logo_reactive_effect>(reactiveEffect));

        float sensitivity = static_cast<float>(_userConfig->getDouble("logo.reactiveSensitivity", 0.5));
        projectm_logo_overlay_set_reactive_sensitivity(_projectMHandle, sensitivity);

        int motionEffect = _userConfig->getInt("logo.motionEffect", 0);
        projectm_logo_overlay_set_motion_effect(_projectMHandle, static_cast<projectm_logo_motion_effect>(motionEffect));

        float motionSpeed = static_cast<float>(_userConfig->getDouble("logo.motionSpeed", 4.0));
        projectm_logo_overlay_set_motion_speed(_projectMHandle, motionSpeed);

        bool randomReactive = _userConfig->getBool("logo.randomReactive", false);
        projectm_logo_overlay_set_random_reactive(_projectMHandle, randomReactive);

        bool randomMotion = _userConfig->getBool("logo.randomMotion", false);
        projectm_logo_overlay_set_random_motion(_projectMHandle, randomMotion);

        float randomInterval = static_cast<float>(_userConfig->getDouble("logo.randomInterval", 15.0));
        projectm_logo_overlay_set_random_interval(_projectMHandle, randomInterval);
    }
    catch (const Poco::Exception& ex)
    {
        poco_warning_f1(_logger, "Error loading persisted logo config: %s", ex.displayText());
    }
}
